...
from deeplab_dpc import common
from deeplab_dpc import model
from deeplab_dpc.datasets import segmentation_dataset
from deeplab_dpc.utils import input_generator
...
flags.DEFINE_string('class_names_file', None,
                    'class_names_file')
...
def main(unused_argv):
    class_names_file = FLAGS.class_names_file
    if class_names_file is None:
        print('class_names_file is None')
        return
    with open(class_names_file, 'r') as load_f:
        ...
        with tf.Graph().as_default():
            ...
            for index in range(0, dataset.num_classes):
                metric_map['class_' + class_names[index] + '_iou'] = (iou_v[index], update_op[index])
            # compute  miou
